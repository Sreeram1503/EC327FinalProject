...
public:
    MemoryGame() : window(sf::VideoMode(1000, 900), "SFML Memory Game") {
        ...
        for (int i = 0; i < SIZE * SIZE; i++) {
            ...
            card.buttonText.setPosition(sf::Vector2f(card.flipButton.getPosition().x + card.flipButton.getSize().x / 2 - textBounds.width / 2,
                                                     card.flipButton.getPosition().y + card.flipButton.getSize().y / 2 - textBounds.height / 2));

            if (i == a_rand || i == a_rand + 1 || i == b_rand || i == b_rand + 1 || i == c_rand || i == c_rand + 1) {
                card.isQuestion = true;
                card.text = (i == a_rand) ? ques_factorial : (i == a_rand + 1) ? ans_factorial
                                                                          : (i == b_rand) ? ques_deriv
                                                                                          : (i == b_rand + 1) ? ans_deriv
                                                                                                             : (i == c_rand) ? ques_radius
                                                                                                                             : (i == c_rand + 1) ? ans_radius : "";
            } else {
                card.text = "";
            }
        ...
        // Shuffle cards
        shuffle(cards.begin(), cards.end(), std::default_random_engine(std::time(0)));
