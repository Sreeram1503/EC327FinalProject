...
public: 
    MemoryGame() { 
        ...
        string ques_hypo = "hypotenuse? sides: " + to_string(side_one) + " , " + to_string(side_two);
            ...
            while (window.pollEvent(event)) {
                ...
                if (event.type == sf::Event::MouseButtonPressed) {
                    if (event.mouseButton.button == sf::Mouse::Left) {
                        ...
                        for (int i = 0; i < 16; i++) {
                            if (cards[i].getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePosition))) {
                                ...
                                if (!cardFlipped[i]) {
                                    ...
                                    cardFlipped[i] = true;
                                        for (int j = 0; j < 8; j++) {
                                            if (questions_answers[j] == questions_answers[j + 8]) {
                                                window.draw(questions_answers_text[j]);
                                            }
                                        }
                                        for (int j = 0; j < 16; j++) {
                                            window.draw(cards[j]);
                                    ...
                                    // flip cards back over since they don't match
                                        cardFlipped[i] = false;
                                        cardFlipped[i + 8] = false;
                                    }

                                    // counts matches to end the game
                                    match_count = 0;
                                    for (int j = 0; j < 8; j++) {
                                        if (questions_answers[j] == questions_answers[j + 8]) {
                                            match_count++;
                                    ...
                                    if (match_count == 8) {
                                        end_screen(window); // clear the window and display "You won!"
                                    }
                                    } else if (questions_answers[i] != questions_answers[i + 8]) {
                                    // flip card back over
                                        cardFlipped[i] = false;
                                    }
                                        hasFlippedCard = false; // reset
                                    } else {
                                        // first card flipped
                                        cardFlipped[i] = true;
                                        hasFlippedCard = true;
                        ...
                        //if time has run out, and the user wants to try again, game resets
                        if(time_left <= 0 and restart_button.getGlobalBounds().contains(static_cast<sf::Vector2f>(mousePosition))){
                            MemoryGame game;
                        }
                    }
                }
