...
//question 8: convert b_rand radians to degrees
    ...
    void countdown_timer(int timeleft, sf::Text countdown){
                int minutes = timeleft / 60;
                int seconds = timeleft % 60;
                std:: string str_min = std::to_string(minutes);
                std::string str_sec = std::to_string(seconds);
                std::string totaltime = str_min + " : " + str_sec;
                countdown.setString(totaltime);
    ...
    void Run() {
        while (window.isOpen())
        {
            ...
            int timeleft = 180;
            countdown_timer(timeleft, countdown);
            if(1 <= timeleft){
                countdown_timer(timeleft,countdown);
